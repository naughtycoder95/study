 ...
 *                  => delete 방식으로 요청되면 대체해서 다른 sql 문이 실행되게 해주는 어노테이션
 *                  => 대체 sql 문 : update 문 추가
 *                  예) delete 요청 -> 대체 sql 문 : update 문 실행
 *                  @SQLDelete(sql ="UPDATE TB_FILE_DB
 *                                        SET DELETE_YN = 'Y'
 *                                      , DELETE_TIME=TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS')
 *                                        WHERE UUID = ?")
 *              3) @Lob : 속성(필드) 위에 붙이는 어노테이션
 *                  => 컬럼에 이미지를 저장하는 자료형 : BLOB (DB)
 *                  => BLOB 자료형에 대응해서 자바에서 제공하는 어노테이션
 *                  예) @Lob
 *                      byte[] 속성명;     // BLOC 자료형 : 이미지
 *                  vs (참고) DB 자료형 : 대량의 문자열 저장하는 자료형 : CLOB
 *                                     일반 문자열 자료형 : varchar2(4000)
 * <p>
 * ===========================================================
 * DATE            AUTHOR             NOTE
 * -----------------------------------------------------------
