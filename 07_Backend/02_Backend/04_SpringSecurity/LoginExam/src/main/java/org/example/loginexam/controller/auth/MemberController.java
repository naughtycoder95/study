...
//                  2) 로그인 버튼 클릭하면 실행될 함수(안만듦, 스프링자동)
...
//        TODO: 1) 매개변수 전달된(jsp 전송) member 가 DB 에 있으면 insert 할 필요 없음 : why? 기존 멤버
        if (memberService.existsById(member.getEmail()) == true) {
            return new RedirectView("/error");  // 에러페이지 강제 리다이렉트
        }
...
//              생성자로 Member 객체 생성 => save 실행
        Member newMember = new Member(
                member.getEmail(),                               // 이메일
                passwordEncoder.encode(member.getPassword()),    // 암호화
                member.getName(),                                // 이름
